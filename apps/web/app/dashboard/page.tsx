...
export default function DashboardPage() {
  ...
  // 2. Quick adjust stock level modal
  ...
  // Helpers for Recipe row adjustments
  const addRecipeRow = () => {
  ...
  // Helpers for Purchase Order row adjustments
  const addPoRow = () => {
    if (ingredientsList.length === 0) return;
  ...
  return (
    <div className={`flex w-full min-h-screen ${t.bg} ${t.text} font-sans antialiased overflow-x-hidden select-none`}>
      ...
      {/* 1. INVENTORY INGREDIENT ADD/EDIT MODAL */}
      {showIngModal && (
